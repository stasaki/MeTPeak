...
# get reads count
my.get.reads.count <- function(ibatch,PARAMETERS,ANNOTATION,ANNOTATION_BATCH_ID,BAM_CHRS,no_bam_files,bam){
  ...
  if (sum(BAM_CHRS==anno$chr)>0) {
    ...
    # compile result
    reads_count = tibble::as_tibble(reads_count)
    reads_count$batch_id=ibatch
    reads_count$check_points=check_points
